...
- **Remove** a built-in row: `disabled: true`. Disabled rows are dropped
  from the effective registry *before* invariant checks, taking their own
  relations with them — so disabling `spec`, `plan` and `retro` together is
  fine even though `retro.closes: plan`. Refused only when an **enabled**
  row's relation targets a disabled one (error names both; clear the
  relation or disable the dependent row in the same overlay).
...
(`RFC-0007`). `id` is engine-reserved: the loader adds it to the `fields`
of any row whose filename carries `{seq}`, the same way branch addressing
adds `branch` — the overlay never has to declare it. `show` resolves either the id or the slug. The slug stays the
...
- `closes: {type, required}` — this type is the closer of `type`. Implies:
  ...
  placeholders or `/` (`"retro.md"`). `doc_path(dt, ident, stage=None)` is
  the only path computation, for every addressing mode: `ident` is the
  identity the row's addressing consumes — the branch (via
  `sanitize_branch`) for branch-addressed rows, the slug for slug-addressed
  rows, ignored for singletons — and `stage` applies to closable rows (and
  `drafts/` for gated ones). A `{seq}` placeholder makes the filename
  unknowable before creation: create allocates the number and formats once;
  later lookups resolve through `iter_docs` matching `id`/slug instead of
  re-formatting. For a closer the path is
  `doc_path(closee, branch, stage).parent / closer.filename`.
...
- **`corpus.py`** (new) — `Doc(path, dt, meta, body)` and
  ...
  paired with its row and parsed frontmatter, plus `doc_path(dt, ident,
  stage=None)` from §2. Lint rules, gates and the dashboard become
